...
The first time you run this, it will save the data into `file_predicts`, and store some metadata in a `*.aux.json` file. Subsequent calls will then check the metadata, and if this looks OK, just load the data in `file_predicts`. This saves you having to repeatedly download the data.
